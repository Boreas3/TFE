...
\quad\ To select the degree of the polynomial such that the function fit the best the given set of operating points, a parametric study over the maximal degree $d$ of the polynomial has been conducted. First, The raw data from Table \ref{tab:C7_compmap} will be used to perform this study.
For each of these polynomial, the value of the objective function $\textbf{F}$ and of the square of the residual function $Res^2$ are computed then compared. The accuracy of the fitting function is described by the value of the objective and residual function. Respectively, a low and high value of these functions implies that the data is well fit by the polynomial.

The results are expressed in Table \ref{tab:C7_regcomp}.

\begin{longtable}[c]{@{}lll|ll@{}}
\caption{Regression result - Compressor map}
\label{tab:C7_regcomp}\\
\toprule
                & \multicolumn{2}{l|}{$\Pi_{tt,c}(\dot{m}_{corr,c},N_c)$} & \multicolumn{2}{l}{$\eta_{is,c}(\Pi_{tt,c},N_c)$} \\* \midrule
\endfirsthead
%
\endhead
%
...
\textbf{1}      & 0.105                       & 0.746                            & 0.073                    & 0.781                          \\
\textbf{2}      & 0.051                       & 0.876                            & 0.018                    & 0.947                         \\
\textbf{3}      & 0.028                       & 0.932                             & 0.009                    & 0.974                          \\
\textbf{4}      & 0.014                       & 0.965                            & 0.008                    & 0.977                          \\
\textbf{5}      & 0.008                       & 0.979                            & 0.007                    & 0.979                           \\
\textbf{6}      & 0.007                       & 0.983                            & 0.007                     & 0.979                          \\
\textbf{7}      & 0.006                       & 0.984                            & 0.007                    & 0.979                          \\* \bottomrule
\end{longtable}

The analysis of the results shows that for the first relation, the degree of the polynomial need to be at least equal to 4 to be enough accurate regarding to the given operating points. 
But, there is still some improvement of the accuracy by increasing up to 6 the degree of the polynomial.

Considering the second relation, starting from a degree equal to 3 there is no significant improvement of the accuracy of the polynomial. 

Figures \ref{fig:C7_polycomp1} and \ref{fig:C7_polycomp2} respectively depicts the interpolating polynomial for the two relations.
...
\begin{figure}[h]
    ...
    \begin{subfigure}[b]{0.4\textwidth}
        \centering
        \includegraphics[width=\textwidth]{/Comp_map/HRPM/calc_132_map.png}
        \caption{$\Pi_{tt,c}(\dot{m}_{corr,c},N_c)$}
        \label{fig:C7_polycomp1}
    \end{subfigure}
    \begin{subfigure}[b]{0.4\textwidth}
        \centering
        \includegraphics[width=\textwidth]{/Comp_map/HRPM/calc_234_map.png}
        \caption{$\eta_{is,c}(\Pi_{tt,c},N_c)$}
...
\subsubsection{Fitting function at low RPM}
\quad\ Now that the method to extend the compressor map to lower rotational speed has been expressed, it would be interesting to verify the validity of the LS-RP for the selected degree of polynomial. To do so, operating points at low RPM have been calculated based on the development described previously. The normalized value are in Table \ref{tab:C7_compmap LRPM}.

\begin{longtable}[c]{@{}llll|llll@{}}
\caption{Normalized compressor map (Low RPM)}
\label{tab:C7_compmap LRPM}\\
\toprule
$\mathbf{\dot{m}_{corr,c,norm}}$ & $\mathbf{\Pi_{tt,c,norm}}$ & $\mathbf{N_{c,norm}}$ & $\mathbf{\eta_{is,c,norm}}$ & $\mathbf{\dot{m}_{corr,c,norm}}$ & $\mathbf{\Pi_{tt,c,norm}}$ & $\mathbf{N_{c,norm}}$ & $\mathbf{\eta_{is,c,norm}}$ \\* \midrule
\endfirsthead
%
...
Considering the same degree for both fitting polynomial, the regression of these points are depicted in Figures \ref{fig:C7_polycomp1_LRPM} and \ref{fig:C7_polycomp2_LRPM}.
...
\begin{figure}[h]
    ...
    \begin{subfigure}[b]{0.4\textwidth}
        \centering
        \includegraphics[width=\textwidth]{/Comp_map/LRPM/calc_142_map.png}
        \caption{$\Pi_{tt,c}(\dot{m}_{corr,c},N_c)$}
        \label{fig:C7_polycomp1_LRPM}
    \end{subfigure}
    \begin{subfigure}[b]{0.4\textwidth}
        \centering
        \includegraphics[width=\textwidth]{/Comp_map/LRPM/calc_245_map.png}
        \caption{$\eta_{is,c}(\Pi_{tt,c},N_c)$}
        \label{fig:C7_polycomp2_LRPM}
    \end{subfigure}
    \caption{Polynomial interpolation of the compressor (Low rotational speed).} \label{fig:C7_polycomp_LRPM}
\end{figure}

As it can be noticed, the relation to obtain the isentropic efficiency is not well conditioned. This is mainly due to the relative proximity of the iso-rotational speeds lines. The relation $\Pi_{tt,c}(\dot{m}_{corr,c},N_c)$ is also affected by this phenomenon but at a lower level.

For both graphs, the regression can be improved by multiplying the pressure ratio $\Pi_{tt,c}$ by the rotational speed $N_c$. Doing this will increase the gap between the lines and will likely improve the overall accuracy of the fitting functions. Figures \ref{fig:C7_polycomp1_LRPM_imp} and \ref{fig:C7_polycomp2_LRPM_imp} shows the improvements resulting from this manipulation.
...
\begin{figure}[h]
    ...
    \begin{subfigure}[b]{0.4\textwidth}
        \centering
        \includegraphics[width=\textwidth]{/Comp_map/LRPM/calc_143_map.png}
        \caption{$[\Pi_{tt,c}\cdot N_c](\dot{m}_{corr,c},N_c)$}
        \label{fig:C7_polycomp1_LRPM_imp}
    \end{subfigure}
    \begin{subfigure}[b]{0.4\textwidth}
        \centering
        \includegraphics[width=\textwidth]{/Comp_map/LRPM/calc_345_map.png}
        \caption{$\eta_{is,c}(\Pi_{tt,c}\cdot N_c,N_c)$}
        \label{fig:C7_polycomp2_LRPM_imp}
    \end{subfigure}
    \caption{Polynomial interpolation of the compressor (part 2 - Low rotational speed).} \label{fig:C7_polycomp_LRPM_imp}
\end{figure}

As depicted in the Figures \ref{fig:C7_polycomp1_LRPM_imp} and \ref{fig:C7_polycomp2_LRPM_imp}, the gap between the curves is larger. This result in a really good estimation of the relation linking the pressure ratio with the corrected mass flow rate and the rotational speed. 

\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{/Comp_map/LRPM/calc_543_map.png}
    \caption{$[\Pi_{tt,c}\cdot N_c](\eta_{is,c},N_c)$}
    \label{fig:C7_polycomp2_LRPM_imp2}
\end{figure}
    
However, the relation to obtain the isentropic efficiency is still not well represented. The reason is that the pressure ratio does not vary much at low rotational speed. Therefore, the polynomial has to fit data that are quasi vertically align which leads to really high slope.

This can be resolve by considering the inverse relation $[\Pi_{tt,c}\cdot N_c](\eta_{is,c},N_c)$. Doing so, the iso-rotational speed curves are horizontal, which is much easier to approach using polynomials. The Figure \ref{fig:C7_polycomp2_LRPM_imp2} illustrates the new polynomial for a degree $d=3$.  However, in order to have a starting guess value for the the isentropic efficiency, the relation $\eta_{is,c}(\Pi_{tt,c},N_c)$ will be used for this purpose.

These operations will be applied on the all the parts of the compressor map. As a summary, the Table \ref{tab:C7_compmaprel} includes for each relations the associated degree of polynomial and the residual values for the low RPM and high RPM data.

\begin{longtable}[c]{@{}llll@{}}
\caption{Compressor map - Table of relations}
\label{tab:C7_compmaprel}\\
\toprule
\multicolumn{1}{c}{\multirow{2}{*}{Relation}} & \multicolumn{1}{c}{\multirow{2}{*}{Degree}} & \multicolumn{2}{c}{$Res^2$} \\
\multicolumn{1}{c}{}                          & \multicolumn{1}{c}{}                        & Low RPM      & High RPM     \\* \midrule
\endfirsthead
%
\endhead
%
...
\endlastfoot
%
$[\Pi_{tt,c}\cdot N_c](\dot{m}_{corr,c},N_c)$ & 6                       & 1.000        & 0.990        \\
$\Pi_{tt,c}(\dot{m}_{corr,c},N_c)$            & 6                       & 1.000        & 0.989        \\
$[\Pi_{tt,c}\cdot N_c](\eta_{is,c},N_c)$      & 3                       & 1.000        & 0.978        \\
$\eta_{is,c}(\Pi_{tt,c},N_c)$                 & 3                       & 0.780766     & 0.974        \\* \bottomrule
\end{longtable}
...
If the inverse and \textit{improved}\footnote{Improved by multiplying the pressure ratio $\Pi_{tt,c}$ by the rotational speed $N_c$.} relation $\dot{m}_{corr,c}(\Pi_{tt,c}\cdot N_c,N_c)$ is considered instead, the choke limit will happen when the derivative of the function will be equal to zeros. To select the degree of the polynomial, a parametric study have been conducted first on the high rotational speed operating points.

\begin{longtable}[c]{@{}lll@{}}
\caption{Regression result - Compressor map (part 2)}
\label{tab:C7_regcomp2}\\
\toprule
\textbf{Degree} & \textbf{F} & $\mathbf{Res^2}$ \\* \midrule
\endfirsthead
%
\endhead
%
...
\textbf{5}      & 0.028   & 0.919         \\
\textbf{6}      & 0.028   & 0.920         \\
\textbf{7}      & 0.028   & 0.920         \\* \bottomrule
\end{longtable}
 The results in Table \ref{tab:C7_regcomp2} shows that a polynomial of degree 4 should be selected. Above, there is not any improvement of the accuracy of the polynomial. The Figure \ref{fig:C7_polycomp3} depicts the functions for 4 rotational speeds.
 
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{Comp_map/HRPM/calc_231_map.png}
    \caption{$\dot{m}_{corr,c}(\Pi_{tt,c}\cdot N_c,N_c)$ - polynomial function.}
    \label{fig:C7_polycomp3}
\end{figure}

As it can be noticed, there are several values of $\Pi_{tt,c}\cdot N_c$ for which the derivative of the function is equal to zero. However, the evaluation of the function $\dot{m}_{corr,c}(\Pi_{tt,c}\cdot N_c,N_c)$ at these points provides close results. Therefore, the selection of the local maximum or local minimum will both provides acceptable results for the choke limit.

This method works well with the high rotational speed operating points. For low rotational speeds curves, the issue express earlier when the relation $\eta_{is,c}(\Pi_{tt,c}\cdot N_c,N_c)$ was considered remains. Another condition for to obtain the maximum flow rate has to be defined. 

It can be demonstrated that for the low rotational speed curves, the choke limit occurs for a \textbf{non normalized} pressure ratio equals to one. Thus, the relation $\dot{m}_{corr,c}(\Pi_{tt,c}\cdot N_c,N_c)$ with $\Pi_{tt,c}=1$ will provides the maximal flow rate.
...
% As for the polynomial function, a study on the degree have been performed. The results, expressed in Table \ref{tab:reg_compmap3}, shows that a trigonometric function of degree 2 is the best choice to be made. Indeed, based on the Figure \ref{fig:C7_trigo} depicting the fitting function, the inaccuracy of the function is mainly for the low mass flow rate. However, this doesn't have any negative impact on the method since the objective is to find the highest corrected mass flow rate for a given rotational speed. 

% \begin{longtable}[c]{@{}lll@{}}
% \caption{Regression result - Compressor map (part 3 - Trigonometric functions).}
% \label{tab:reg_compmap3}\\
% \toprule
%                 & \multicolumn{2}{l}{$\dot{m}_{corr,c}(\Pi_{tt,c},N_c)$} \\* \midrule
% \endfirsthead
% %
% \endhead
% %
...
% \textbf{3}      & 0.028                         & 0.919                              \\
% \textbf{4}      & 0.028                         & 0.920                               \\
% \textbf{5}      & 0.028                         & 0.921                               \\* \bottomrule
% \end{longtable}
...
% For the low rotational speed, the choke limit wil
\subsection{Turbine maps}
\quad\ From now, only the compressor maps have been considered. However, the extrapolation of the turbine maps using a fitting function is also required in order to characterize the turbine operating point. In the beginning of the current section, it was said that the two relations that need to be obtained using the least-square regression algorithm are (\ref{eq:C7_turbMCbis}) and (\ref{eq:C7_turbETAbis}.

\begin{subequations}
\setstretch{1}
\begin{equation}
    \dot{m}_{corr,t} = \dot{m}_{corr,t}(\Pi_{tt,t},N_t)\label{eq:C7_turbMCbis}
\end{equation}
\begin{equation}
    \eta_{is,t} = \eta_{is,t}(\Pi_{tt,t},N_t)\label{eq:C7_turbETAbis}
\end{equation}
\end{subequations}

As  for the compressor maps, a parametric study on the degree of the polynomial has to be performed in order to select the best candidate for the polynomial regression. The set of operating points on which the study will be performed is given in the Table \ref{tab:C7_turbmap}.
...
The results of the study are included in the Table \ref{tab:reg_turbmap1}. 
\begin{longtable}[c]{@{}lll|ll@{}}
\caption{Regression result - Turbine map}
\label{tab:reg_turbmap1}\\
\toprule
                & \multicolumn{2}{l|}{$\dot{m}_{corr,t}(\Pi_{tt,t},N_t)$} & \multicolumn{2}{l}{$\eta_{is,t}(\Pi_{tt,t},N_t)$} \\* \midrule
\endfirsthead
%
\endhead
%
...
\endlastfoot
%
\textbf{Degree} & \textbf{F}                       & $\mathbf{Res^2}$                       & F                          & $\mathbf{Res^2}$                   \\
\textbf{1}      & 0.055                          & 0.753                               & 0.108                   & 0.070                           \\
\textbf{2}      & 0.013                         & 0.943                               & 0.052                   & 0.550                           \\
\textbf{3}      & 0.002                         & 0.992                               & 0.018                  & 0.847                           \\
\textbf{4}      & 2.55E-04                         & 0.999                               & 0.004                   & 0.967                           \\
\textbf{5}      & 7.12E-05                         & 0.999                               & 0.001                   & 0.990                            \\
\textbf{6}      & 3.32E-05                         & 0.999                               & 6.98E-04                  & 0.993                           \\* \bottomrule
\end{longtable}

From these results, it can be seen that a polynomial of degree 6 provide good results for the relation defining the isentropic efficiency of the expansion. The Figure \ref{fig:C7_etaturb} depicts the polynomial obtain for this relation.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{/Turb_map/calc_356_map.png}
    \caption{$\eta_{is,t}(\Pi_{tt,t},N_t)$ - polynomial function}
    \label{fig:C7_etaturb}
\end{figure}
Considering now the first relation, the value of the objective function $\mathbf{F}$ and of the square of the residual function $Res^2$ shows that the fitting is nearly perfect for a polynomial of degree 4. 

\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{/Turb_map/calc_351_map.png}
    \caption{$\dot{m}_{corr,t}(\Pi_{tt,t},N_t)$ - polynomial function}
    \label{fig:C7_mturb1}
\end{figure}
However, when the graphical representation in Figure \ref{fig:C7_mturb1} of the polynomial is analyzed, the first notice is that the points for the different iso-rotational speeds are really close from one to the other. Thus, while the of the point is perfect, the results that the function will provide outside the given domain will be disastrous. 

\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{/Turb_map/calc_452_map.png}
    \caption{$\dot{m}_{corr,t}\cdot\Pi_{tt,t}](\Pi_{tt,t}\cdot N_t,N_t)$ - polynomial function}
    \label{fig:C7_mturb2}
\end{figure}

To solve this problem, a method needs to be found to increase the gap between the iso-rotational speeds. The retained solution consists modifying the relation (\ref{eq:C7_turbMCbis}) as given in the new relation (\ref{eq:C7_turbMCbis_new}).

\begin{equation}
\setstretch{1}
    [\dot{m}_{corr,t}\cdot\Pi_{tt,t}](\Pi_{tt,t}\cdot N_t,N_t)\label{eq:C7_turbMCbis_new}
\end{equation}

Starting from this new relation, the spacing between the different curves is increased. It results in a quasi straight polynomial as illustrated in Figure \ref{fig:C7_mturb2}. 


The degree selected of this polynomial is 3 according to the results from the table \ref{tab:reg_turbmap2}. 

\begin{longtable}[c]{@{}lll@{}}
\caption{Regression result - Turbine map (part 2)}
\label{tab:reg_turbmap2}\\
\toprule
                & \multicolumn{2}{l}{$\dot{m}_{corr,t}\cdot\Pi_{tt,t}](\Pi_{tt,t}\cdot N_t,N_t)$} \\* \midrule
\endfirsthead
%
\endhead
%
...
\textbf{Degree} & \textbf{F}                          & $\mathbf{Res^2}$                          \\
\textbf{1}      & 0.020                               & 0.974                                     \\
\textbf{2}      & 1.21E-03                            & 0.999                                     \\
\textbf{3}      & 1.79E-04                            & 1.000                                     \\
\textbf{4}      & 3.76E-05                            & 1.000                                     \\
\textbf{5}      & 1.5E-05                             & 1.000                                     \\
\textbf{6}      & 7.61E-06                            & 1.000                                     \\
\textbf{7}      & 7.79E-06                            & 1.000                                     \\* \bottomrule
\end{longtable}