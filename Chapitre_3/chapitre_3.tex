...
The second family are turbomachines dealing with compressible flow. Here, some extract effects are taken into account. For instance, the velocity of the flow can be at some points limited due to choking effects. These effects will be described in a future part of this section.
...
\quad\ Some principles used when designing incompressible and compressible flow based machines are common for both of these families. Among these principles, it can be emphasized the velocity triangle and the similarity analysis. The first provides tools to well design the blades of the turbomachines, while the last principles is used to derive the performance of a given machines based on non-dimensional quantities. 
...
\begin{figure}[h]
    ...
    \label{fig:C3_frames}
\end{figure}
Figure \ref{fig:C3_frames} shows the representations in the space of these different definitions of the velocity. Considering a point at $P$ in the time t=0, this point is moving to \(P\)'' in the time t=dt.  
...
\quad\ When the study of turbomachine is considered, experimental testing are required to accurately design it. However, due to the high cost of creating a new test bench for each designed machines, the principle of similarity have been developed. 

The similarity analysis allows to extrapolate the performance of a given turbomachine based on non-dimensional quantities. Indeed, when moving from on turbomachine to a similar one, the value of the non-dimensional quantities are conserved. Similarly, operating points can be extrapolated using similarity.

This is a really powerful tool which allows to significantly decrease the time and the cost related to the design of a new turbomachines.
...
\quad\ As it has been partly revealed in the previous lines, the flow characteristics are dependent on its velocity. The study of compressible flow did lead to the distinction between the static and total quantities.
...
The total enthalpy can be defined as “the static enthalpy obtained when the gas is brought adiabatically to a halt” \cite{Hillewaert2019}.
...
\end{equation}
This ratio depends on the type of fuel used. For instance, if the fuel was propane (\ce{C3H8}), the ratio would be equal to 5.
...
\begin{figure}[h]
    \centering
    \subfloat[HX with fined plates \cite{Ngendakumana2018}\label{fig:C3_HX_fin_plate}]{\includegraphics[width=0.4\textwidth]{HX_fin_plate}}\hfill
    \subfloat[HX with fined tubes \cite{Ngendakumana2018}\label{fig:C3_HX_fin_tube}] {\includegraphics[width=0.4\textwidth]{HX_fin_tube}}\hfill
    \subfloat[Plate heat-exchangers \cite{Ngendakumana2018}\label{fig:C3_PHE}]{\includegraphics[width=0.4\textwidth]{HX_brased_plate}}
...
\begin{figure}[h]
    \centering
    \subfloat[Temperature profil for parallel flow HX \cite{Ngendakumana2018}\label{fig:C3_HX_par_flow_T}]{\includegraphics[width=0.45\textwidth]{parallele_flow_T}}\hfill
    \subfloat[Temperature profil for counter flow HX \cite{Ngendakumana2018}\label{fig:C3_HX_opo_flow_T}] {\includegraphics[width=0.45\textwidth]{opposite_flow_T}}\caption{Temperature profiles for different stream configurations}\label{fig:C3_Tprof}
\end{figure}
